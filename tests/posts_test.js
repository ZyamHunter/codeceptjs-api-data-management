...
const { validationBody, validationBodyArray } = require('../components/validationData');
...
Scenario('test Factory matches', () => {
    ...
    let fac = Factory.build('matches', { seasonStart: '2016-03-12' }, { numMatches: 3 });
...
Scenario('Get comments of one post',  async ({ I }) => {
    try {
        const response = await I.sendGetRequest(`/posts/${postId}/comments`, headers);
        I.seeResponseCodeIsSuccessful()
        I.seeResponseCodeIs(200)
...
Scenario('Creat post',  async ({ I }) => {
    ...
    try {
        const response = await I.sendPostRequest(`/posts/${postId}/comments`, payload, headers);
...
Scenario('Update post completely',  async ({ I }) => {
    ...
    try {
        const response = await I.sendPutRequest(`/posts/${postId}`, payload, headers);
        I.seeResponseCodeIsSuccessful()
        I.seeResponseCodeIs(200)
...
Scenario('Update item in post',  async ({ I }) => {
    ...
    try {
        const response = await I.sendPatchRequest(`/posts/${postId}`, payload, headers);
        I.seeResponseCodeIsSuccessful()
        I.seeResponseCodeIs(200)
        I.seeResponseContainsJson(payload)

...
module.exports = {
    ...
    validationBody(payload, response) {
        for (const key of Object.keys(payload)) {
            assert.equal(response[key], payload[key], `O valor de ${key} na resposta não corresponde ao valor enviado.`);
        }
    },
    ...
    validationBodyArray(array, response) {
        for (const key of array) {
            assert.ok(response.hasOwnProperty(key), `O campo ${key} não está presente na resposta.`);
        }
    }
}